...
export async function getPosSessionVersion(env) {
    const row = await env.DB.prepare("SELECT value FROM meta WHERE key = 'posSessionVersion'").first();
    return row ? String(row.value) : "1";
}
...
// ---- POS (cashier) auth: separate token namespace (kind:'pos') ----
// Verifies a cashier token and enforces posSessionVersion.
export async function authenticatePos(request, env) {
    if (!env.AUTH_SECRET) return null;
    const payload = await verifyToken(env.AUTH_SECRET, getBearer(request));
    if (!payload || payload.kind !== "pos") return null;
    const sv = await getPosSessionVersion(env);
    if (String(payload.sv) !== sv) return null;
    return payload;
}

// Gate a POS write. Accepts a valid cashier token OR an admin back-office token
// (so the admin can also exercise POS endpoints). role optional for cashier.
export async function requirePos(request, env, role) {
    const pos = await authenticatePos(request, env);
    if (pos) {
        if (role && pos.role !== role) return { error: bad(403, "forbidden"), user: null };
        return { error: null, user: pos };
    }
    const admin = await authenticate(request, env);
    if (admin) return { error: null, user: admin };
    return { error: bad(401, "unauthorized"), user: null };
}
